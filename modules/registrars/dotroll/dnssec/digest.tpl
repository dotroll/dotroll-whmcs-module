	...
	<form id="zeditorform" class="form-horizontal" method="post" action="{$smarty.server.PHP_SELF}?action=domaindetails&id={$domainid}&modop=custom&a=Dnssec">
		<div class="center100">
			<table class="table table-striped table-framed">
				<thead>
					<tr>
						<th class="textleft">Key Tag</th>
						<th class="textleft">Algorithm</th>
						<th class="textleft">Digest Type</th>
						<th class="textleft">Digest</th>
						<th></th>
					</tr>
				</thead>
				...
				<tfoot>
					<tr id="addtr" style="display: none;">
						<td><input disabled="disabled" type="text" name="newrecords_keytag" class="form-control input-sm" value=""/></td>
						<td>
						...
						<td>
							<select disabled="disabled" name="newrecorddigesttype" class="form-control input-sm">
								{foreach from=$digestTypes item=digesttype key=digesttypeid}
									<option value="{$digesttypeid}">{$digesttype}</option>
								{/foreach}
							</select>
						</td>
						<td><input disabled="disabled" type="text" name="newrecords_digest" class="form-control input-sm" value=""/></td>
